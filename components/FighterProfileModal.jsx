  ...
  return (
    <>
      ...
      <button
        ...
        className="w-10 h-10 rounded-xl bg-slate-100 hover:bg-slate-200 dark:bg-zinc-900 dark:hover:bg-zinc-800 border border-slate-200 dark:border-zinc-800 flex items-center justify-center text-slate-700 dark:text-zinc-350 transition-all cursor-pointer shadow-sm relative group"
          ...
          <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-3xl w-full max-w-xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200">
            ...
            <div className="flex items-center justify-between p-6 border-b border-slate-100 dark:border-zinc-800">
              ...
              <button
                ...
                className="p-2 rounded-xl text-slate-400 hover:bg-slate-100 dark:hover:bg-zinc-800 hover:text-slate-650 dark:hover:text-zinc-200 transition-all cursor-pointer"
              >
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-2 gap-5">
                <div>
                  ...
                  <span className="text-sm font-extrabold text-red-500 dark:text-red-400 uppercase tracking-wide block mt-0.5">{fighter.style}</span>
                </div>
                ...
                <div>
                  ...
                  <span className="text-sm font-bold text-slate-700 dark:text-zinc-250 block mt-0.5">{fighter.experienceLevel}</span>
                </div>

                <div>
                  <span className="block text-[10px] font-bold text-slate-400 dark:text-zinc-550 uppercase tracking-wider">Weight Class</span>
                  <span className="text-sm font-bold text-slate-700 dark:text-zinc-250 block mt-0.5">{fighter.weightClass}</span>
                </div>
                ...
                <div>
                  ...
                  <span className="text-sm font-bold text-slate-700 dark:text-zinc-250 block mt-0.5 font-mono">{formatDate(fighter.joiningDate)}</span>
                </div>

                <div>
                  <span className="block text-[10px] font-bold text-slate-400 dark:text-zinc-550 uppercase tracking-wider">Phone</span>
                  <span className="text-sm font-bold text-slate-700 dark:text-zinc-250 font-mono block mt-0.5">{fighter.phone}</span>
                </div>

                <div>
                  <span className="block text-[10px] font-bold text-slate-400 dark:text-zinc-550 uppercase tracking-wider">Email Address</span>
                  <span className="text-sm font-bold text-slate-700 dark:text-zinc-250 font-mono block mt-0.5 break-all">{fighter.email}</span>
                </div>
              </div>

              {/* Billing Cycle Details */}
              <div className="border-t border-slate-100 dark:border-zinc-800/80 pt-5 mt-5">
                <h4 className="text-xs font-bold uppercase tracking-wider text-slate-450 dark:text-zinc-500 mb-3 flex items-center gap-1.5">
              ...
              <div className="border-t border-slate-100 dark:border-zinc-800/80 pt-5 mt-5">
                ...
                {fighter.assignedCoach ? (
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h5 className="text-xs font-extrabold text-slate-800 dark:text-zinc-150 uppercase">{fighter.assignedCoach.name}</h5>
