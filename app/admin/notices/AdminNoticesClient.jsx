    ...
    <div className="space-y-8 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 bg-gradient-to-r from-slate-900 to-zinc-900 dark:from-zinc-900 dark:to-black rounded-3xl p-6 md:p-8 text-white shadow-xl border border-slate-800 dark:border-zinc-800">
        ...
        <div className="shrink-0 flex gap-4 text-xs font-black uppercase tracking-widest">
          <div className="px-4 py-3 bg-slate-800/50 dark:bg-zinc-900/60 border border-slate-700/50 dark:border-zinc-800/80 rounded-2xl">
            <span className="block text-slate-400 text-[8px] font-bold tracking-wider">TOTAL NOTICES</span>
      ...
      {initialEvents.length === 0 ? (
        <div className="text-center py-20 bg-white dark:bg-zinc-900/50 border border-slate-200 dark:border-zinc-800 rounded-3xl p-8">
          ...
          <h3 className="text-base font-bold text-slate-600 dark:text-zinc-300 uppercase tracking-tight">No Notices Posted Yet</h3>
          ...
          {initialEvents.map((event) => (
            ...
            >
              <div className="space-y-4">
                ...
                <p className="text-xs text-slate-500 dark:text-zinc-400 font-medium leading-relaxed line-clamp-3">
                  {event.description}
                </p>

                {/* Audio announcement if exists */}
                {event.audio && (
                  <div className="flex items-center gap-2 p-2 bg-slate-50 dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 rounded-xl w-fit" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 rounded-3xl w-full max-w-3xl shadow-2xl overflow-hidden animate-in fade-in zoom-in-95 duration-200 flex flex-col" style={{ maxHeight: '90vh' }}>
            ...
            <div className="flex items-center justify-between p-6 border-b border-slate-100 dark:border-zinc-800 shrink-0 gap-2">
              <div className="flex items-center gap-2.5 min-w-0 flex-1">
                ...
                <h3 className={`font-extrabold text-xs sm:text-base uppercase tracking-tight truncate min-w-0 flex-1 ${getTitleColorClass(activeEvent.titleColor)}`}>
                  {activeEvent.title}
                </h3>
              </div>
              <button
              ...
              <div className="p-6 md:w-1/2 space-y-5 overflow-y-auto">
                {activeEvent.image && (
                  <div className="w-full rounded-2xl overflow-hidden border border-slate-200 dark:border-zinc-800 shadow-sm relative bg-slate-100 dark:bg-zinc-950/50">
                    <img src={activeEvent.image} alt={activeEvent.title} className="w-full object-contain max-h-56 bg-slate-50 dark:bg-zinc-950" />
                  </div>
                )}
                ...
                <div className="flex-1 overflow-y-auto space-y-3 min-h-0">
                  {activeModalTab === 'comments' && (
                    <div className="flex flex-col h-full">
                      ...
                      {/* Comment submit form */}
                      <form onSubmit={(e) => handleCommentSubmit(e, activeEvent._id)} className="flex gap-2 items-center mt-auto pt-2 border-t border-slate-100 dark:border-zinc-800 shrink-0">
                        <input
                          ref={commentInputRef}
                          type="text"
                          value={commentText}
